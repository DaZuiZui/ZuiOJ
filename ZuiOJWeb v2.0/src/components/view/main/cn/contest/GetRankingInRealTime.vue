<template>
  ...
  export default {
       ...
       mounted(){
            this.webSocketIp = globle.WebSocketIp;
            this.webSocketPort = globle.MainPort;
            ...
            this.getRanking(1);
               ...
               // 设置定时器，每隔一秒向后端发送请求
                setInterval(() => {
                    const message = this.curpage; // 替换为您的消息内容
       ...
       methods: {
            ...
            setupWebSocket() {
                ...
                this.socket = new WebSocket("ws://"+this.webSocketIp+":"+this.webSocketPort+"/api/zuioj/"+getQueryVariable("contestId")+"/1/"+25);
            ...
            async getRanking(page){
                ...
                if (this.socket && this.socket.readyState === WebSocket.OPEN) {
                    ...
                    console.error("WebSocket connection is not in OPEN state.");
                }
